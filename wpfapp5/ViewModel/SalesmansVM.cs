...
namespace StarNote.ViewModel
{
    public class SalesmansVM : BaseModel
    {
        ...
        private void DataValidChecker()
        {
            while (true)
            {
                if (MainWindow.ActivePage == MainWindow.AppPages.Salesman || MainWindow.ActivePage == MainWindow.AppPages.SalesmanEdit)
                {
                    if (!isDataValid)
                    {
                        Loaddata();
        ...
        private void Preparenewsave()
        {
            if (UserUtils.Authority.Contains(UserUtils.Üründetay_Ekle))
            {
                ...
                Savebtnvisibility = Visibility.Visible;
                Updatebtnvisibility = Visibility.Hidden;
                MainWindow.ChangePage(MainWindow.AppPages.SalesmanEdit);
            }
            else
            {
        ...
        public void Save()
        {
            try
            {
                ...
                if (isok)
                {
                    MainWindow.ChangePage(MainWindow.AppPages.Salesman);
                    Loaddata();
