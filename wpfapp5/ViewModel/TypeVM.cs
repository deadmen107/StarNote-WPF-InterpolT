...
namespace StarNote.ViewModel
{
    public class TypeVM : BaseModel
    {
        ...
        private void DataValidChecker()
        {
            while (true)
            {
                if (MainWindow.ActivePage == MainWindow.AppPages.Type || MainWindow.ActivePage == MainWindow.AppPages.TypeEdit)
                {
                    if (!isDataValid)
                    {
                        Loaddata();
        ...
        private void Preparenewsave()
        {
            if (UserUtils.Authority.Contains(UserUtils.Üründetay_Ekle))
            {
                ...
                Savebtnvisibility = Visibility.Visible;
                Updatebtnvisibility = Visibility.Hidden;
                MainWindow.ChangePage(MainWindow.AppPages.TypeEdit);
            }
            else
            {
        ...
        public void Save()
        {
            try
            {
                ...
                if (isok)
                {
                    MainWindow.ChangePage(MainWindow.AppPages.Type);
                    Loaddata();
