    ...
    public partial class DailySalesUC : UserControl
    {
        ...
        private void Btnpdf_ItemClick(object sender, DevExpress.Xpf.Bars.ItemClickEventArgs e)
        {
            if (UserUtils.Authority.Contains(UserUtils.Günlük_satıs_yazdırma))
            {
                try
                {
                    LogVM.Addlog(this.GetType().Name, System.Reflection.MethodBase.GetCurrentMethod().Name, "INFO", "Günlük Satış Rapor İsteği alındı", "");
                    PrintingRoute printingRoute = new PrintingRoute();
                    string msg = string.Empty;
                    msg += printingRoute.DailySales;
                    if (printingRoute.DailySales == "")
                    {
                    ...
                    else
                    {
                        ...
                        if (result == MessageBoxResult.Yes)
                        {
                            ...
                            links[0].ExportToPdf(printingRoute.DailySales + "\\Günlük Satış " + DateTime.Now.ToString("dd MM yyyy HH mm") + ".pdf");
                            LogVM.Addlog(this.GetType().Name, System.Reflection.MethodBase.GetCurrentMethod().Name, "INFO", "Günlük Satış Rapor alındı ", "");
                ...
                catch (Exception ex)
                {
                    ...
                    LogVM.Addlog(this.GetType().Name, System.Reflection.MethodBase.GetCurrentMethod().Name, "ERROR", "Günlük Satış Rapor hatası ", ex.Message);
                }
            }
            else
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (result == MessageBoxResult.Yes)
                        {
                            List<TemplatedLink> links = new List<TemplatedLink>();
                            links.Add(new PrintableControlLink((TableView)grdsatınalma.View) { Landscape = true });
                            links[0].ExportToXlsx(printingRoute.DailySales + "\\" + RaporAdı + " Raporu " + DateTime.Now.ToString("dd MM yyyy HH mm") + ".xlsx");
                        }
                    }
                }
                catch (Exception ex)
        ...
        private void columnvisiblechange(object sender, RoutedEventArgs e)
        {
            ...
            DailySalesUI settings = new DailySalesUI();
